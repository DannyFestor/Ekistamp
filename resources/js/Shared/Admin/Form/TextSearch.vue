<template>
  <section ref="textsearch" class="relative flex flex-col">
    <label :for="id"><slot /></label>
    <input
      class="relative"
      type="text"
      @keydown.esc.tab="showOptions = false"
      :value="props.search"
      ...
      <article v-if="showOptions" class="absolute rounded bg-white shadow">
        ...
        <div
          @click="
            ...
            label = [option.name, option.romaji].filter((n) => n).join(' - ');
            showOptions = false;
          "
...
const clickOutsideEvent = (e) => {
  showOptions.value = !!(
    textsearch.value === e.target || textsearch.value.contains(e.target)
  );
};
onMounted(() => {
